...
public sealed class Duel() : IsekaiHeroCard(1, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
{
    ...
    [
        ...
        new DamageVar("SoloDamage", 6m, ValueProp.Move)
    ];

    public override List<(string, string)> Localization => new CardLoc(
        "Duel",
        "# Deal !Damage! damage. If only one enemy remains, deal !SoloDamage! more damage.");
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay play)
    {
        ...
        if (IsConditionalEffectActive(combatState.HittableEnemies.Count() == 1))
            damage += DynamicVars["SoloDamage"].BaseValue;
    ...
    protected override void OnUpgrade()
    {
        ...
        DynamicVars["SoloDamage"].UpgradeValueBy(2m);
    }
}
